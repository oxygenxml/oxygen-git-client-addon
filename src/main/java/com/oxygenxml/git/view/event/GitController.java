...
public class GitController extends GitControllerBase {
	...
	protected void showPullSuccessfulWithConflicts(PullResponse response) {
		...
		FileStatusDialog.showWarningMessage(translator.getTranslation(Tags.PULL_STATUS), conflictingFilesList,
	...
	protected void showPullFailedBecauseOfCertainChanges(List<String> filesWithChanges, String message) {
		...
		FileStatusDialog.showWarningMessage(translator.getTranslation(Tags.PULL_STATUS), filesWithChanges, message);
	}

	/**
	 * Show the "Rebase in progress" dialog. It allows the user to continue or abort
	 * the rebase.
	 */
	...
	private abstract class ExecuteCommandRunnable implements Runnable {
		...
		/**
		 * Executes the command. If authentication is to be tried again, the method will
		...
		private void executeCommand() {
			...
			try {
				...
				if (cause instanceof org.eclipse.jgit.errors.CheckoutConflictException) {
					String[] conflictingFile = ((org.eclipse.jgit.errors.CheckoutConflictException) cause)
							.getConflictingFiles();
					showPullFailedBecauseOfCertainChanges(Arrays.asList(conflictingFile),
					...
					// It's a pretty serious exception. Present it in a dialog so that the user
					// takes measures.
					...
					event = Optional.of(new PushPullEvent(getOperation(),
							composeAndReturnFailureMessage(lockFailedException.getMessage()), e));
				} else {
					// It's a pretty serious exception. Present it in a dialog so that the user
					// takes measures.
					PluginWorkspaceProvider.getPluginWorkspace().showErrorMessage(e.getMessage(), e);
					event = Optional
							.of(new PushPullEvent(getOperation(), composeAndReturnFailureMessage(e.getMessage()), e));
				}
			} catch (RebaseUncommittedChangesException e) {
				showPullFailedBecauseOfCertainChanges(e.getUncommittedChanges(),
						translator.getTranslation(Tags.PULL_REBASE_FAILED_BECAUSE_UNCOMMITTED));
			} catch (RebaseConflictsException e) {
				showPullFailedBecauseOfCertainChanges(e.getConflictingPaths(),
						translator.getTranslation(Tags.PULL_REBASE_FAILED_BECAUSE_CONFLICTING_PATHS));
				...
				PluginWorkspaceProvider.getPluginWorkspace()
						.showErrorMessage(translator.getTranslation(Tags.PUSH_FAILED) + " "
								+ translator.getTranslation(Tags.PUSH_FAILED_TRANSPORT_EXCEPTION), e);
			} catch (GitAPIException e) {
				// Exception handling.
				boolean shouldTryAgain = AuthUtil.handleAuthException(e, hostName,
						exMessage -> PluginWorkspaceProvider.getPluginWorkspace().showErrorMessage(exMessage), true);
				if (shouldTryAgain) {
		 ...
		 */
		protected abstract Optional<PushPullEvent> doOperation(CredentialsProvider credentialsProvider)
				throws GitAPIException;
	}
	...
	private class ExecutePushRunnable extends ExecuteCommandRunnable {
		...
		/**
		 * Push the changes and inform the user with messages depending on the result
		 * status.
		 * 
		 * @param credentialsProvider The credentials provider.
		 * 
	...
	private class ExecutePullRunnable extends ExecuteCommandRunnable {
		...
		/**
		 * Pull the changes and inform the user with messages depending on the result
		 * status.
		 * 
		 * @param credentialsProvider The credentials provider.
		 * 
		...
		protected Optional<PushPullEvent> doOperation(CredentialsProvider credentialsProvider) throws GitAPIException {
			...
			if (repositoryState != null) {
				if (repositoryState == RepositoryState.MERGING_RESOLVED) {
					...
					PullResponse response = gitAccess.pull(credentialsProvider, pullType,

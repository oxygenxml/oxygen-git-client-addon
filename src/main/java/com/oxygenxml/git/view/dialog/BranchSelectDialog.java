...
public class BranchSelectDialog extends OKCancelDialog {
	...
	private void addSelectBranchLabel(GridBagConstraints gbc) {
		...
		gbc.weightx = 0;
		gbc.weighty = 0;
		JLabel label = new JLabel(translator.getTranslation(Tags.BRANCH) + ": ");
    ...
    branchesCombo.setEnabled(false);
	  ...
	  GitOperationScheduler.getInstance().schedule(() -> {
	    try {
	      ...
	      SwingUtilities.invokeLater(() -> BranchSelectDialog.super.doOK()); // NOSONAR
	    }
	  });
	}
